...
const MOCK_LEADS = [
  { id: 'lead1', name: 'Karim Alami', email: 'karim.alami@gmail.com', phone: '+212 661 123 456', status: 'Chaud', lastContact: '02/08/2026 : Rappel pour signature', projectOfInterest: 'Résidence ATLAS' },
  { id: 'lead2', name: 'Sara Bennani', email: 'sara.bennani@hotmail.com', phone: '+212 662 987 654', status: 'Nouveau', lastContact: '01/08/2026 : Intéressée par Villa Océan', projectOfInterest: 'Villa Océan' },
  { id: 'lead3', name: 'Youssef Filali', email: 'youssef.filali@yahoo.fr', phone: '+212 663 456 789', status: 'Tiède', lastContact: '28/07/2026 : A visité le témoin', projectOfInterest: 'Résidence ATLAS' }
];
...
export default function CommercialDashboard({ navigation, route }) {
  ...
  const [commissionInfo, setCommissionInfo] = useState({ totalSales: 0, estimatedComm: 0 });
  ...
  const fetchCommercialData = async () => {
    ...
    try {
      ...
      console.error("Erreur de récupération des données commercial:", err.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const calculateCommissions = (allLots) => {
    ...
    const myLots = allLots.filter(l => 
      ...
      (l.commercial_id === user.id || user.role === 'admin' || user.role === 'promoteur')
    );

    const totalSales = myLots.reduce((acc, curr) => acc + Number(curr.price), 0);
  ...
  const handleGenerateContract = (lot) => {
    ...
    const clientOptions = leads.map(l => ({
      text: l.name,
      onPress: () => processContractGeneration(lot.id, '3da5b6fa-9011-4770-983b-f458e0a1c1d0' || user.id, lot.number) // Utilise un ID client simulé ou celui du conseiller
    }));
    ...
    Alert.alert(
      ...
      [
        ...clientOptions.map(opt => ({
          text: opt.text,
          onPress: () => processContractGeneration(lot.id, '3da5b6fa-9011-4770-983b-f458e0a1c1d0', lot.number) // Simule l'id client acquéreur
        })),
  ...
  const processContractGeneration = async (lotId, clientId, lotNumber) => {
    ...
    try {
      ...
      Alert.alert(
        ...
        [
          { text: 'Télécharger / Ouvrir', onPress: () => Linking.openURL(res.pdfUrl) },
          { text: 'Plus tard' }
        ]
      );
  ...
  const renderLotItem = ({ item }) => {
    ...
    return (
      <View style={styles.lotCard}>
        ...
        <Text style={styles.lotProject}>{item.project?.name || 'Résidence ATLAS'}</Text>
        <Text style={styles.lotType}>Modèle : {item.type} | Surface : ~100m²</Text>
        <Text style={styles.lotPrice}>{Number(item.price).toLocaleString()} DH</Text>

        {item.client && (
          <View style={styles.clientDetail}>
            <Text style={styles.clientLabel}>Acquéreur : {item.client.full_name || item.client.email}</Text>
            <Text style={styles.clientPhone}>Tél : {item.client.phone || '-'}</Text>
          </View>
        )}
        ...
        {item.status === 'available' && (
          ...
          >
            {isProcessing ? (
              ...
              <Text style={styles.actionBtnText}>Réserver & Éditer contrat (Express PDF)</Text>
            )}
          </TouchableOpacity>
        )}
      </View>
    ...
    return (
      <View style={styles.leadCard}>
        <View style={styles.leadHeader}>
          ...
          <View style={[
            styles.leadBadge,
            item.status === 'Chaud' ? styles.leadChaud : item.status === 'Nouveau' ? styles.leadNouveau : styles.leadTiede
          ]}>
            <Text style={[
              styles.leadBadgeText,
              item.status === 'Chaud' ? { color: colors.danger } : item.status === 'Nouveau' ? { color: colors.roles.ingenieur } : { color: colors.roles.promoteur }
            ]}>{item.status.toUpperCase()}</Text>
          </View>
        </View>

        <Text style={styles.leadContact}>📧 {item.email} | 📞 {item.phone}</Text>
        <Text style={styles.leadProject}>Projet d'intérêt : {item.projectOfInterest}</Text>
        ...
        <TouchableOpacity 
          ...
          onPress={() => Alert.alert('Action CRM', `Interaction client enregistrée pour ${item.name}.`)}
        >
  ...
  if (loading) {
    return (
      <SafeAreaView style={styles.loadingContainer}>
        ...
        <Text style={styles.loadingText}>Chargement du catalogue commercial...</Text>
      </SafeAreaView>
    );
  }
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.tabRow}>
        ...
        >
          <Text style={[styles.tabButtonText, activeTab === 'lots' && styles.tabButtonTextActive]}>Lot Grid & Ventes</Text>
        </TouchableOpacity>
        <TouchableOpacity 
          style={[styles.tabButton, activeTab === 'crm' && styles.tabButtonActive]}
          onPress={() => setActiveTab('crm')}
        >
          <Text style={[styles.tabButtonText, activeTab === 'crm' && styles.tabButtonTextActive]}>Mini-CRM Leads</Text>
        </TouchableOpacity>
      </View>
      ...
      <ScrollView contentContainerStyle={styles.scrollContainer}>
        ...
        <View style={styles.commissionCard}>
          ...
          <Text style={styles.commSub}>Basée sur vos ventes validées et réservations en cours</Text>
          
          <View style={styles.commMetricRow}>
            <View>
              <Text style={styles.commVal}>{commissionInfo.estimatedComm.toLocaleString()} DH</Text>
              <Text style={styles.commLabel}>Commission Estimée</Text>
            </View>
            <View style={{ alignItems: 'flex-end' }}>
              <Text style={styles.commSalesVal}>{commissionInfo.totalSales.toLocaleString()} DH</Text>
          ...
          <Text style={styles.commTarget}>Objectif Commission Trimestrielle : 100,000 DH</Text>
        </View>
          ...
          <View>
            ...
            <View style={styles.filtersContainer}>
                ...
                {['all', 'available', 'reserved', 'sold'].map(status => (
                  ...
                  >
                    <Text style={[
                      ...
                      {status === 'all' ? 'Tous' : status === 'available' ? 'Dispo' : status === 'reserved' ? 'Réservé' : 'Vendu'}
                    </Text>
                  </TouchableOpacity>
                ))}
              </View>
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.projectFilterRow}>
                <TouchableOpacity
                  style={[styles.projectFilterTag, selectedProjectId === 'all' && styles.projectFilterTagActive]}
                  onPress={() => setSelectedProjectId('all')}
                >
                  <Text style={[styles.projectFilterTagText, selectedProjectId === 'all' && { color: colors.background }]}>Tous les Projets</Text>
                </TouchableOpacity>
                {projects.map(p => (
                  <TouchableOpacity
                    key={p.id}
                    style={[styles.projectFilterTag, selectedProjectId === p.id && styles.projectFilterTagActive]}
                    onPress={() => setSelectedProjectId(p.id)}
                  >
                    <Text style={[styles.projectFilterTagText, selectedProjectId === p.id && { color: colors.background }]}>{p.name}</Text>
                  </TouchableOpacity>
                ))}
              </ScrollView>
            </View>
            ...
            <FlatList
              ...
              scrollEnabled={false}
              ListEmptyComponent={
                <Text style={styles.emptyText}>Aucun lot correspondant à ces filtres.</Text>
              }
            />
          </View>
        ) : (
          ...
          <View>
            <View style={styles.alertBanner}>
              ...
              <View style={{ flex: 1, marginLeft: 10 }}>
                ...
                <Text style={styles.alertDesc}>Rappeler Karim Alami aujourd'hui concernant le compromis de vente.</Text>
              </View>
            </View>

            <Text style={styles.sectionHeader}>Portefeuille Clients Actifs</Text>
            <FlatList
              data={leads}
              keyExtractor={(item) => item.id}
