...
export default function ClientDashboard({ navigation, route }) {
  ...
  const fetchClientData = async () => {
    ...
    try {
      ...
      const currentProj = projRes.projects[0] || { id: 'default', name: 'Résidence ATLAS', location: 'Casablanca', latitude: 33.5731, longitude: -7.5898 };
      ...
      if (currentProj.id) {
        ...
        if (list.length > 0) {
          const totalProgress = list.reduce((acc, cur) => acc + Number(cur.progress_percent), 0);
        ...
        fetchWeather(currentProj.latitude || 33.5731, currentProj.longitude || -7.5898);
      }
  ...
  const handleOpenReceipt = (url) => {
    if (!url) {
      Alert.alert('Indisponible', 'Le reçu de ce paiement n\'a pas encore été généré ou téléversé.');
      return;
    }
    Linking.openURL(url).catch(() => Alert.alert('Erreur', 'Impossible d\'ouvrir l\'adresse URL.'));
  };

  const handleScheduleMeeting = () => {
    if (!meetingDate || !meetingTime || !meetingSubject) {
      Alert.alert('Champs requis', 'Veuillez saisir une date, une heure et le motif du rendez-vous.');
      return;
    }
    setScheduling(true);
    setTimeout(() => {
  ...
  if (loading) {
    return (
      <SafeAreaView style={styles.loadingContainer}>
        ...
        <Text style={styles.loadingText}>Chargement de votre espace acquéreur...</Text>
      </SafeAreaView>
    );
  }
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContainer}>
        ...
        <View style={styles.sectionCard}>
          ...
          <Text style={styles.projectName}>{project?.name} - {project?.location}</Text>
          
          {/* Jauge d'avancement globale */}
          <View style={styles.progressRow}>
            <Text style={styles.progressText}>Avancement global : {progressPercent}%</Text>
          ...
          {milestones.length === 0 ? (
            <Text style={styles.emptyText}>Aucun jalon défini pour ce projet.</Text>
          ) : (
            milestones.map((item, idx) => (
              <View key={item.id} style={styles.timelineItem}>
                <View style={styles.timelineIndicators}>
                  <View style={[
                    styles.timelineDot,
                    item.is_validated ? styles.timelineDotActive : styles.timelineDotInactive
                  ]} />
                ...
                <View style={styles.timelineContent}>
                  <Text style={[
                    ...
                    item.is_validated ? styles.textActive : styles.textInactive
                  ]}>
          ...
          {weatherLoading ? (
            ...
            <View style={styles.weatherInfoRow}>
              <View>
                ...
                <Text style={styles.weatherLocation}>Localisation GPS : {project?.latitude}, {project?.longitude}</Text>
              </View>
              <View style={styles.weatherDetailRight}>
                <Text style={styles.weatherText}>Vents : {weather.windspeed} km/h</Text>
                <Text style={styles.weatherStatus}>
                  {weather.temperature > 30 ? 'Chaud / Favorable' : weather.temperature > 15 ? 'Clément' : 'Frais'}
                </Text>
              </View>
            </View>
          ) : (
        ...
        <View style={styles.sectionCard}>
            ...
            <View style={styles.gaugeContainer}>
              <View style={styles.gaugeStats}>
                <View style={styles.gaugeStatBox}>
                  <Text style={styles.gaugeVal}>{Number(paymentMetrics.totalLotValue).toLocaleString()} DH</Text>
                ...
                <View style={styles.gaugeStatBox}>
                  <Text style={[styles.gaugeVal, { color: colors.roles.promoteur }]}>
          ...
          {payments.length === 0 ? (
            <Text style={styles.emptyText}>Aucune facture ou appel de fonds émis.</Text>
          ) : (
            payments.map((p) => {
              ...
              return (
                <View key={p.id} style={styles.paymentRow}>
                  <View style={{ flex: 1 }}>
                    <Text style={styles.paymentTitle}>Étape Chantier (10%)</Text>
                    <Text style={styles.paymentAmount}>{Number(p.amount).toLocaleString()} DH</Text>
                    <Text style={styles.paymentDue}>Échéance : {new Date(p.due_date).toLocaleDateString('fr-FR')}</Text>
                  </View>
                  <View style={{ alignItems: 'flex-end' }}>
                    <View style={[
                      ...
                      <Text style={[
                        ...
                        isPaid ? { color: colors.success } : isOverdue ? { color: colors.danger } : { color: colors.warning }
                      ]}>
        ...
        <View style={styles.sectionCard}>
          ...
          <Text style={styles.cardDescription}>Documents officiels et plans chantiers sécurisés en lecture seule.</Text>
          
          {documents.length === 0 ? (
            <Text style={styles.emptyText}>Aucun document déposé dans votre coffre-fort.</Text>
          ) : (
            documents.map((doc) => (
              <TouchableOpacity 
                key={doc.id} 
              ...
              >
                ...
                <View style={{ flex: 1, marginLeft: 12 }}>
                  ...
                  <Text style={styles.docMeta}>Format : PDF | Déposé le {new Date(doc.created_at).toLocaleDateString('fr-FR')}</Text>
                </View>
        ...
        <View style={styles.sectionCard}>
          ...
          <Text style={styles.cardDescription}>Prenez rendez-vous directement avec Amine Bennani (Sales Agent).</Text>
